...
export default function AdminEditOrderModal({ order, onClose, onSaved }: Props) {
  ...
  // No-Charge is applied immediately per item (persists server-side) — tracked
  // locally so the row updates without closing the modal mid-edit.
  const [ncState, setNcState] = useState<Record<string, boolean>>({});
  const [ncBusy, setNcBusy] = useState<string | null>(null);
  const isNc = (it: { _id: string; isNoCharge?: boolean }) =>
    ncState[it._id] ?? !!it.isNoCharge;

  async function toggleNc(it: { _id: string; name: string; isNoCharge?: boolean }) {
    const currentlyNc = isNc(it);
    let reason: string | undefined;
    if (!currentlyNc) {
      const r = window.prompt(`Mark "${it.name}" No-Charge — reason?`, "");
      if (r === null) return;
      reason = r || undefined;
    }
    setNcBusy(it._id);
    try {
      const res = await fetch(`/api/orders/${order._id}/items/${it._id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ noCharge: !currentlyNc, reason }),
      });
      if (!res.ok) throw new Error();
      setNcState((p) => ({ ...p, [it._id]: !currentlyNc }));
      toast.success(currentlyNc ? "No-Charge removed" : "Marked No-Charge");
    } catch {
            ...
            <div className="space-y-1.5">
              {existing.map((it) => {
                ...
                return (
                  <div
                    key={it._id}
                    className={`flex items-center gap-2 text-sm rounded-lg px-2 py-1.5 bg-base-200 ${removed ? "opacity-40 line-through" : ""}`}
                  >
                    ...
                    {!removed && (
                      <button
                        onClick={() => toggleNc(it)}
                        disabled={ncBusy === it._id}
                        title={isNc(it) ? "Remove No-Charge" : "Mark No-Charge (₹0)"}
                        className={`btn btn-xs ${isNc(it) ? "btn-success" : "btn-ghost border border-base-300"}`}
                      >
                        NC
                      </button>
                    )}
